...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($email === '' || $password === '') {
        ...
        $stmt = $conn->prepare("SELECT id_utilisateur, nom, prenom, email, mot_de_passe, role FROM utilisateur WHERE email = ?");
        $stmt->bind_param('s', $email);
        $stmt->execute();
        $stmt->store_result();
        if ($stmt->num_rows === 1) {
            $stmt->bind_result($id, $nom, $prenom, $e, $hash, $role);
            $stmt->fetch();
            if (password_verify($password, $hash)) {
                ...
                $authError = "Cet email n’existe pas ou le mot de passe est incorrect.";
            }
        } else {
            $authError = "Cet email n’existe pas ou le mot de passe est incorrect.";
        }
...
<body>
    ...
    <section class="p-5" style="background-color: #f0f5fa;">
        ...
        <div class="card card-body mx-auto mt-4 rounded-4" style="width: fit-content;">
            <h6>Connexion</h6>
            <p class="text-secondary">Entrez vos identifiants pour continuer</p>
            <?php if ($error): ?>
            ...
            <small class="text-danger mt-1 d-block"><?php echo htmlspecialchars($authError); ?></small>
            <?php endif; ?>
            ...
            <div class="d-flex mx-auto">
                <p>Vous n'avez pas de compte ?</p><a href="inscrire.php">Inscrivez-vous ici</a>
            </div>
            <p class="text-secondary text-center">Vous n'avez pas de compte ? <a href="inscrire.php">Créez-en un</a></p>
